...
The HUD displays information in a panel on the viewport.

> **Warning (Houdini 21.0):** The `hudInfo()` method may cause crashes due to a Qt overlay bug. Use `print()` or `setPromptMessage()` instead for debugging. See [Troubleshooting](troubleshooting.md) for details.
...
Display a message in the viewport status bar (more reliable than HUD):
