...
**Valid parameters for point drawables:**
- `radius` (float): Point size in world units. Default: 0.05
- `color1` (tuple): RGBA color
- `style` (hou.drawableGeometryPointStyle): Point shape
...
def on_selection_changed(selection):
    for node in selection.nodes():
        if node.type().name() == "th::click_counter::1.0":
            viewer = hou.ui.paneTabOfType(hou.paneTabType.SceneViewer)
            viewer.setCurrentState("th_click_counter")
            break
...
node_type = hou.nodeType(hou.sopNodeTypeCategory(), "th::click_counter::1.0")
